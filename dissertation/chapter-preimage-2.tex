...
\begin{comment}
Jay: These sections are a bit too much code with a bit too little
revelation about the code. Maybe because they are appendix-like it
doesn't matter. What did you have in mind for them?
\end{comment}
...
From implementations of multiplication restricted to each open quadrant, or $mul^{++}\pre$, $mul^{+-}\pre$, $mul^{-+}\pre$ and $mul^{--}\pre$, we define multiplication on all of $\Re \times \Re$ with
\begin{equation}
\begin{aligned}
	mul\pre &\ := \
		\lzfcsplit{\ifpre~
...
\begin{comment}
Jay: Is Example 8.29 meant to explain or impress? I think it fails to
explain a bit.

Me: Primarily to impress (to show off flexibility), secondarily to demonstrate
that most of the objects in the sampling theorem aren't actually realized in
programs. Did it achieve my secondary objective?

Jay: The theory doesn't feel cool enough to impress that much. It feels
like it's going to be simple and I'll understand it, then when I
don't, it makes the system look worse because it seems confusing.
\end{comment}

\begin{comment}
Jay: It is not obvious why (a) and (b) in the figure have
different darkness. I presume it is density or probability after
normalizing? (b) gets shallower? Why is that?

Me: They're plots of samples, so the overall darkness depends only on the size
and transparency of each point. (If you zoom in on either of these plots in
DrRacket, for example, they look light and sparse.) In each plot,
technically, only the relative darkness in any given area matters.

I used different point transparencies to make each plot look nice
separately. If I use the same point transparency for both, plot (b) gets
pretty dark inside the dashed box. You can think of the reason as density
renormalization, because plotting the samples using filled, partially
transparent circles is very similar to estimating the density from samples
by adding together uniform-height cylinders. More cylinders near any
coordinate makes its density estimate larger; more filled circles near it
makes the plot darker there. But drawing with alpha transparency is only
approximately additive...

I can try using the same alpha for the points in both if you think it'll
match your expectations better.

Jay: Maybe the real point is that you don't explain in the writing (a) what
we should expect these graphs to look like and (b) why what they
actually look like indicates that you did a good job.
\end{comment}

\begin{figure}[!tb]
\centering
\subfloat[Samples chosen according to overlapping candidate distributions $Q~left$ and $Q~right$]{%
\label{fig:pimp-sampling-2d:unweighted}%
\includegraphics[width=3in]{figures/pimp-sampling-unweighted}%
}%
\hspace{0.1in}%
\subfloat[After resampling candidate samples by weight]{%
\label{fig:pimp-sampling-2d:resampled}%
\includegraphics[width=3in]{figures/pimp-sampling-weighted}%
}%
\caption[Sampling uniformly in a partioned unit square]{Partitioned importance sampling used to sample uniformly in a partition of the unit square, using two overlapping, overapproximating candidate distributions.}
...
\begin{figure*}[tb!]\centering
\subfloat[The computation tree for $if~(random < random)~\mathit{e_1}~\mathit{e_2}$.]{%
\includegraphics[width=3.5in]{figures/expression-tree}%
}%
\tab\tab%
\subfloat[The partition induced by maximal branch traces $T^*$.
Each part is labeled with a trace that induces it.]{%
\includegraphics[width=2.5in]{figures/trace-induced-partition}%
}%
...
In fact, only traces in $T^*$ induce nonempty subsets of $\Omega$.
...
Let $m : Set~\Re \pto [0,\infty]$ be Lebesgue measure on $\Re$ (i.e. length).
Define
\begin{equation}
\begin{aligned}
...
\caption[Independent vs. dependent uniform sampling]{$500$ samples using Dr. Bayes's implementations of $sample!part$, $sample!source$ and $sample!source^*$.}
...
In general, for the interpretation of \scheme{diagonal}, the proportion of accepted samples in the preimage of $[-\varepsilon,\varepsilon]$ scales linearly with $\varepsilon$.
