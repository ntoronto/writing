...
We can mitigate this problem using finer partitions of $proj~j~\Omega'$, but there is a better solution.

The key insight is that the singleton interval $[b,b] = \set{b}$ is also a rectangle.
For each axis $j \in J'$, $sample!source$ may choose a $b \in proj~j~\Omega'$, update $\Omega'$ using $unproj~j~Omega'~\set{b}$, and use $refine$ to get better bounds for sampling the other axes.
...
The caller is expected to linearize $J'$ into $js : [J]$, the indexes of random values that are actually used while running the program.
Dr. Bayes's implementation of $sample!part$ returns $js$ in addition to the covering rectangle and its probability.

Figure~\ref{fig:uniform-sample-source:nonuniform} shows the results of using Dr. Bayes's implementation of $sample!source^*$ to sample within the parts of the same partition.
