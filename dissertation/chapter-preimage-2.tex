...
\begin{itemize}
	\item It samples from a rectangular cover of a partition of $\Omega \times T$ instead of from a rectangular equivalence class of traces.
	\item It handles $random!idxs~j$ by applying $sample!real!part$ to $proj~j~\Omega'$.
...
Subfigures (b) and (c) demonstrate the difference between a uniform and nonuniform $sample!source$ implementation.
In (b), 255 samples are accepted; in (c), all 500 samples are accepted.}
\label{fig:uniform-sample-source}
\end{figure*}


Figure~\ref{fig:uniform-sample-source} shows the result of sampling within the preimage of $\Re \times \Re \times [-0.05,0.05]$ under the interpretation of this program as a random variable:
\begin{center}\singlespacing
\begin{schemedisplay}
(define/drbayes e
  (let ([x  (random)]
...
XXX: need a nonuniform sampler

XXX: key insight: a singleton is a rectangle, so we can sample $a$ and $unproj~j~\Omega'~\set{a}$ and use $refine$ to get bounds for other random values

XXX: code

XXX: extension: no $\bot$ branch choices when sampling in $T_+$; characterize the programs for which this extension doesn't terminate; explain options to detect or fix
