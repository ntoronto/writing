...
These are regarded as nondeterministic, but with additional information that quantifies the likelihoods of the values they may take on.
The additional information is completely characterized by a function called a \keyword{joint distribution}.

For example, suppose $X,Y \in \set{h,t}$ are random variables that represent the outcomes of coin tosses.
Further, let the joint distribution $p_{X,Y} : \set{h,t} \times \set{h,t} \to [0,1]$ quantify the likelihood of every possible combination of outcomes by defining
\begin{equation}
...
In general, the result of a probability query $\Pspec{e}$ is the sum of probabilities of outcomes for which $e$ is true.
...
To compute the probability that $Y = t \implies X = t$, we apply logical rules to the query until it is a disjunction of distinct outcomes, and add their probabilities:
\begin{equation}
\begin{aligned}
...
Probability mass functions cannot quantify the likelihoods of uncountably many outcomes, such as when $X \in \Re$.
...
For any random variable $X \in \Re$, value $x \in \Re$, and density function $f : \Re \to [0,+\infty)$,
\begin{equation}
	\Pspec{X = x}\ =\ \int_x^x f(x)\, dx\ =\ (f(x) - 0) \cdot (x - x)\ =\ f(x) \cdot 0\ =\ 0
\end{equation}
...
In both examples, on a part of the distribution's domain that has length or area $0$ (i.e. the single outcome $100$ or the set $\setb{x \in \Re, y \in \Re}{\sqrt{x^2+x^2} = z}$), the probability of that set is nonzero.
Integrating on such a set can yield only $0$, so the distributions cannot be defined by densities.

Measure theory's answer is to define probability distributions using \keyword{probability measures}, which map \emph{sets to probabilities} instead of \emph{values to instantaneous changes} in probability.
...
XXX: segue: abstract interpretation embraces infinities and infinite computation in the concrete semantics; in this case the objective is to define the abstract semantics as a computable, sound approximation of the concrete; learn something about programs by running them approximately on infinite sets of inputs (e.g. type checking); to make the concrete semantics a categorical semantics, we need a powerful $\lambda$-calculus
