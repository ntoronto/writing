...
It mainly does this by assigning probabilities to sets instead of points.
Functions that do so are \keyword{probability measures}.

If a probability measure $P$ assigns probabilities to subsets of $X$ and $g : X \to Y$, then the distribution over subsets of $Y$ is defined by
\begin{equation}
	\Pr[B] \ = \ P~(preimage~g~B)
\end{equation}
...
The proofs of these theorems are in Appendix~\ref{ch:measurability}.
