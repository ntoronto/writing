...
Any general-purpose programming language can generate pseudorandom values, but a \emph{probabilistic} programming language can allow finding the probabilistic conditions under which stated constraints are satisfied.
Often, the program simulates a real-world random process and the constraints represent observed, real-world outcomes.
Running a simulating probabilistic program under observation constraints \emph{infers cause from effect}.

Inferring probabilistic causes from observed outcomes is \keyword{Bayesian inference}, which is used heavily in artificial intelligence and has been successful in analyzing phenomena at all scales, from genomes to celestial bodies.
...
Probabilistic programming languages that are implemented using advanced Monte Carlo algorithms might be used not just for Bayesian inference, but for \emph{simulating rare events} by encoding the events' criteria as constraints.
...
But the theory's generality makes finding useful computational content difficult.
...
\begin{enumerate}
	\item The $preimage$ function can operate only on \emph{extensional} functions, which have observable domains, such as $\fun{r \in [0,1]} max~\pair{0.5,a}$. In particular, it cannot operate on lambdas such as $\fun{r} max~\pair{0.5,a}$ (even if they are typed).%
