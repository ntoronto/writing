...
\begin{itemize}
	...
	\item $X^J$ is the type of extensional, total functions from set $J$ to set $X$; i.e. vectors of $X$ indexed by $J$. ($J$ is called an \keyword{index set}.)
\end{itemize}
...
While $arr\prehat$ is not unique nor computable, specific applications of it are (see \figref{fig:extra-preimage-arrow-defs}).]{
\begin{minipage}{0.98\textwidth}
\begin{align*}
...
\subfloat[Abstract preimage arrow lifts needed to interpret probabilistic programs.]{
\begin{minipage}{0.98\textwidth}
\begin{align*}
\\ % add space between subfloat above and this one
\begin{aligned}[t]
...
A lift $lift\prehat : (X \botto Y) \to (X \prehatto Y)$ exists, but it is not unique nor computable; we thus do not define $arr\prehat$ in terms of it.
