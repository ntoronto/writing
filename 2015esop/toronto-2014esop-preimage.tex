...
\centering
\includegraphics[width=.75\textwidth]{ray-tracing.pdf}
\end{minipage}
...
\centering
\includegraphics[width=.7\textwidth]{ray-tracing-projection.png}
\end{minipage}
\label{fig:ray-tracing-projection}
}
\caption[ ]{Ray tracing by constraining the outputs of a probabilistic program.\captionsquash}
...
For example, consider $max~\pair{0.5,random}$, where $max~\pair{a,b}$ returns the greater of the pair $\pair{a,b}$.
...
\begin{enumerate}
	\item A \keyword{standard semantics} $\meaningof{\cdot}_\bot$ that interprets programs that may raise errors (e.g. divide-by-zero) as intensional functions.
...
Second, they allow us to \emph{derive} all the arrows for recursive, probabilistic programs at once, by lifting the arrows for nonrecursive, nonprobabilistic programs.
...
A fifth combinator allows lazy branching in a call-by-value language such as \lzfclang.
...
\begin{equation}%
\begin{CD}%
X \botto Y   @>lift\pre>>   X \preto Y \\[-4pt]%
@V{\eta_\pbot}VV              @VV{\eta\ppre}V\\[-6pt]%
...
\end{align*}
\bottomhrule
\caption[ ]{Bottom arrow definitions.\captionsquash}
...
\end{align*}
\bottomhrule
\caption[ ]{Preimage functions and operations.\captionsquash}
...
\end{align*}
\bottomhrule
\caption[ ]{Preimage arrow definitions.\captionsquash}
...
%One easy way to identify a particular node in an infinite tree is by the path from the root to the node.
...
\end{align*}
\bottomhrule
\caption[ ]{$AStore$ (associative store) arrow transformer definitions.\captionsquash}
...
Suppose $f := (\meaningofconv{\mathit{p}}_\pbot~j_0) : X' \botto Y$ and $h := (\meaningofconv{\mathit{p}}\ppre~j_0) : X' \preto Y$, where $X' = (R \times T) \times X$.
...
Interpreting conditionals requires $\set{true}$ and $\set{false}$; thus $Rect~Bool ::= Set~Bool$.
...
The lift $arr\prehat : (X \to Y) \to (X \prehatto Y)$ exists, but $arr\prehat~f$ is not always unique (because by definition, $Rect~X^J$ is an incomplete lattice) nor computable.
...
It includes $\meaningof{\cdot}\genc$ (\figref{fig:semantic-function}), its extension $\meaningof{\cdot}\genc\conv$, the bottom* arrow (\figsref{fig:bottom-arrow-defs} and~\ref{fig:astore-arrow-defs}), the abstract preimage and preimage* arrows (\figsref{fig:abstract-arrow-defs} and~\ref{fig:astore-arrow-defs}), and other manual lifts to compute abstract preimages under real functions such as arithmetic, $sqrt$ and $log$.
The abstract preimage arrows operate on a monomorphic rectangular set data type, which includes tagged rectangles and disjoint unions for ad-hoc polymorphism, and floating-point intervals to overapproximate real intervals.
...
We therefore cannot easily represent floating-point numbers in Dr.~Bayes---but we do not want to.
...
Kozen's seminal work~\cite{cit:kozen-1979fcs-prob-programs-short} on probabilistic semantics defines two measure-theoretic, denotational semantics, in two different styles: a \keyword{random-world semantics}~\cite{cit:mcallester-2008tr-random-world} that interprets programs as deterministic functions that operate on a random source, and a \keyword{distributional semantics} that interprets programs as probability measures.
