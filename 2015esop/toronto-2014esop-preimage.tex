...
\begin{aligned}[t]
	&\begin{aligned}[t]
		&X \pprehatto Y ::= AStore~(R \times T)~(X \prehatto Y)
	\end{aligned} \\
\\[-6pt]
	&\begin{aligned}[t]
		&random\pprehat : X \pprehatto [0,1] \\
		&\lzfcsplit{&random\pprehat~j \ := \ \\ &\tab fst\pre~\acomp\prehat~fst\pre~\acomp\prehat~\pi\pre~j}
	\end{aligned} \\
\\[-6pt]
	&\begin{aligned}[t]
		&branch\pprehat : X \pprehatto Bool \\
		&\lzfcsplit{&branch\pprehat~j \ := \ \\ &\tab fst\pre~\acomp\prehat~snd\pre~\acomp\prehat~\pi\pre~j}
	\end{aligned} \\
\\[-6pt]
	&\begin{aligned}[t]
		fst\pprehat &:= \eta\pprehat~fst\pre;\ \cdots
	\end{aligned}
\end{aligned}
&
\begin{aligned}[t]
