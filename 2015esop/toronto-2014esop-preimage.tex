...
We use $(\arrowpair)$ to apply each of these interpretations to the random source to create a pair, and $(\arrowcomp)$ to send the pair to $max$.
Thus, $max~\pair{0.5,random}$, interpreted as a function arrow computation, is $f := ((const~0.5)~\arrowpair~id)~\arrowcomp~max$.

By substituting the definitions of $const$, $id$, $(\arrowpair)$ and $(\arrowcomp)$, we would find that $f$ is equivalent to $\fun{r} max~\pair{0.5,r}$, similar to the interpretation in~\eqref{eqn:max-random-manual-interp}.
...
\begin{aligned}
	f~r
	&\ =\ (((\fun{r}0.5)~\arrowpair~id)~\arrowcomp~max)~r
	\tab && \textrm{def. of $f$}
\\
	&\ =\ ((\fun{r} \pair{(\fun{r}0.5)~r,id~r})~\arrowcomp~max)~r
	\tab && \textrm{def. of $(\arrowpair)$}
\\
	&\ =\ ((\fun{r} \pair{0.5,r})~\arrowcomp~max)~r
	&& \textrm{$\beta$ reduce}
\\
...
\begin{equation}
	f_a\ :=\ (arr_a~(const~0.5)~\arrowpair_a~arr_a~id)~\arrowcomp_a~arr_a~max
\end{equation}
