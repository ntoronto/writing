...
\end{equation}
Here, $find!hit~(fst~path)~dir$ find the surface the photon hits.
If the photon is absorbed, $find!hit$ returns a data structure containing just the collision point $pt$.
If reflected, $find!hit$ returns a data structure containing the collision point $pt$ and surface normal $norm$, which $random!half!dir$ uses to choose a new direction.
...
The head of the path (the last collision point) is constrained to be in the aperture.
...
\begin{enumerate}
	...
	\item Deriving an approximating semantics, which allows computing answers to questions about probabilistic programs to arbitrary accuracy.
...
Arrows factor first-order computation into the following tasks.
\begin{enumerate}
...
The last combinator allows lazy branching in a call-by-value metalanguage.
...
\begin{definition}[arrow\footnote{\arrowfootnote}]A binary type constructor $(\arrow\gen)$ and the combinators
\begin{displaybreaks}
\begin{equation*}
\begin{aligned}
...
\begin{aligned}
	...
	\arrowif~f_1~f_2~f_3~a &\ := \ if~f_1~a~then~f_2~a~else~f_3~a \\
	\arrowlazy~f~a &\ :=\ f~0~a
\label{eqn:function-arrow}
\end{aligned}
\end{equation}
Suppose we want to interpret the program $max~\pair{0.5,random}$ as a function arrow computation.
To do so compositionally, we must interpret every subexpression as a function arrow computation.
For any random source $r$, the interpretation of $0.5$ should return $0.5$; therefore $0.5$ means $\fun{r}0.5$.
If we assume $r \in [0,1]$, then $random$ means $\fun{r} r$, or $id$.
...
Thus, $max~\pair{0.5,random}$, as a function arrow computation, is
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
which is equivalent to the manual interpretation of $max~\pair{0.5,random}$ in~\eqref{eqn:max-random-manual-interp}.
...
If $\arrowlift\genb : (x \arrow\gen y) \to (x \arrow\genb y)$ is a right-invertible homomorphism from arrow $a$ to arrow $b$ and the arrow laws hold for $a$, then the arrow laws hold for $b$.
\end{theorem}
\begin{proof}
...
\\
	\meaningof{if~\mathit{e_c}~then~\mathit{e_t}~else~\mathit{e_f}}\gen &\ :\equiv\
		\arrowif\gen~
			...
			(\arrowlazy\gen~\fun{0}{\meaningof{\mathit{e_f}}\gen})\hspace{-3in}
\end{aligned}
&\tab\tab\tab\tab\ 
\begin{aligned}[t]
...
\\
	\meaningof{\mathit{f}~\mathit{e}}\gen &\ :\equiv\
		\meaningof{\pair{\mathit{e},\pair{}}}\gen~\arrowcomp\gen~\mathit{f}
\\
	\meaningof{\delta~\mathit{e}}\gen &\ :\equiv\
		\meaningof{\mathit{e}}\gen~\arrowcomp\gen~\arrowarr\gen~\delta
\\
...
The result of applying $\meaningof{\cdot}\gen$ is a \lzfclang program in \emph{environment-passing style} with the environment represented by a stack.
...
Using De Bruijn indexes, the definition $f~x := f~x$ is written $f := f~(env~0)$, which $\meaningof{\cdot}\gen$ interprets as $f := \meaningof{\pair{env~0,\pair{}}}\gen~\arrowcomp\gen~f$.
In a call-by-value metalanguage, this circular definition is divergent.
To avoid this tangential issue and ill-typed expressions like $max~\pair{0.5,\pair{}}$, we assume all programs are \keyword{well-defined}.

\begin{definition}[well-defined]
\label{def:well-defined-expression}
An expression (or program) $\mathit{e}$ is \keyword{well-defined} under arrow $a$ if $\meaningof{\mathit{e}}\gen : x \arrow\gen y$ for some $x$ and $y$, and $\meaningof{\mathit{e}}\gen$ terminates.
\end{definition}

Well-definedness does not guarantee that \emph{running} an interpretation terminates.
We assume it because it simplifies statements about expressions, such as the following theorem, on which most of our semantic correctness results rely.
...
\begin{enumerate}
	...
	\item Define $\arrowlift\genb : (x \arrow\gen y) \to (x \arrow\genb y)$ from arrow $a$ to arrow $b$ in such a way that $\arrowlift\genb~f$ for any $f : x \arrow_a y$ is obviously correct.
...
Operations $\pair{\cdot,\cdot}\pre$ and $(\circ\pre)$ return preimage functions that compute preimages under pairing and composition, and are derived from the preimage identities in~\eqref{eqn:preimage-identities}; $(\u\pre)$ computes unions and is used to define $\ifpre$.
...
\\
	&\lazypre~h~A \ := \ if~A = \emptyset~then~\emptyset\pre~else~h~0~A
\\
	&\liftpre \ := \ pre
\end{aligned}
\end{align*}
...
\begin{aligned}
	\meaningofconv{if~\mathit{e_c}~then~\mathit{e_t}~else~\mathit{e_f}}\genc &\ :\equiv\
		\arrowconvif\genc~
			...
			(\arrowlazy\genc~\fun{0}{\meaningofconv{e_t}\genc})~
			(\arrowlazy\genc~\fun{0}{\meaningofconv{e_f}\genc})
\end{aligned}
\end{equation}
		...
		&\lzfcsplit{
			...
			&\tab\lzfclet{
				...
				A_3 & p_k~C_3 \i p_b~C_3 \\
					...
					&else~(k_2~(left~(right~j))~A_2 \u\pre' k_3~(right~(right~j))~A_3)\!\!\!\!\!\!\!\!\!}}
		}
	\end{aligned}
\end{aligned}
\end{align*}
...
%Poole~\cite{cit:poole-2010-ppls} advocates using random-world semantics as a way to allow constraining deterministic functions of random values.
