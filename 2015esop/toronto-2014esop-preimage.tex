...
\begin{itemize}
	...
	\item $X^J$ is the type of extensional, total functions from set $J$ to set $X$; i.e. vectors of $X$ indexed by $J$. ($J$ is called the \keyword{index set}.)
\end{itemize}
...
For a type $X$ of language values, $Rect~X$ denotes the type of \mykeyword{rectangular sets} of $X$: a bounded lattice ordered by $(\subseteq)$; i.e. it contains $\emptyset$ and $X$, and is closed under intersection $(\i)$ and join $(\join)$.
...
While $arr\prehat$ is not computable, specific applications of it are (see \figref{fig:extra-preimage-arrow-defs}).]{
\begin{minipage}{0.98\textwidth}
\begin{align*}
\\[-6pt]
\begin{aligned}[t]
