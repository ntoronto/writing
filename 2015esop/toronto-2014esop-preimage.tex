...
For example, the function arrow is defined by $x \arrow y ::= x \to y$ and
\begin{equation}
\begin{aligned}
	\arrowarr~f &\ := \ f \\
...
So far, we have ignored the many arrow laws, which ensure that arrows are well-behaved (e.g. effects are correctly ordered) and are useful in proofs of theorems in which arrows are parameters (i.e. nothing else is known about them).
Fortunately, we can prove all the laws for an arrow $a$ by defining it in terms of an arrow $b$ for which the laws hold, and proving two properties about the lift from $a$ to $b$.
...
\\
	\delta &\ ::\equiv \ \text{[primitive functions]}
\\[-6pt]
\end{align*}
\begin{align*}
\begin{aligned}[t]
...
Function definitions may be mutually recursive because they are interpreted as definitions in a lambda calculus in which such recursion is already supported.
(We thus do not need to define an explicit fixpoint operator.)
Unlike functions, local variables are unnamed.
They are referred to by De Bruijn indexes, with $0$ referring to the innermost binding.
...
Using De Bruijn indexes, the circular definition $f~x := f~x$ is written $f := f~(env~0)$, which $\meaningof{\cdot}\gen$ interprets as $f := \meaningof{\pair{env~0,\pair{}}}\gen~\arrowcomp\gen~f$.
In a call-by-value lambda calculus, this self-referential interpretation is divergent.
Other issues may arise with ill-typed expressions such as $5+\pair{}$.
To avoid these tangential issues, we assume all programs are \keyword{well-defined}.
