...
Using De Bruijn indexes, $g~x := g~x$ is written $g := g~(env~0)$, which $\meaningof{\cdot}\gen$ interprets as $g := \meaningof{\pair{env~0,\pair{}}}\gen~\arrowcomp\gen~g$.
...
\begin{enumerate}
	...
	\item Define $\arrowlift\genb : (x \arrow\gen y) \to (x \arrow\genb y)$ from arrow $a$ to $b$ in such a way that $\arrowlift\genb~f$ for any $f : x \arrow_a y$ is obviously correct.
